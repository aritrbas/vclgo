...
> design record of what was built and why. Paths and launch instructions in
> the body are archival and are not present in the current tree. The shipping
> backend is documented in
> [the current architecture](../../architecture.md); its diagrams are in
...
> for the current owner model are in
...
The archived Approach #3 implementation did not attempt to be a complete Linux socket ABI
virtualizer. It focuses on the syscall subset exercised by ordinary Go TCP
clients and servers. This is historical scope, not a statement about current
support; see [current status](../../status.md).
